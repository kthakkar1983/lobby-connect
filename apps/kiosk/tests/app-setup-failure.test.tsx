...
// startCall() resolves (row created, callIdRef set), fetchAgoraToken rejects.
...
describe("App onStartCall — setup-failure catch closes the created row", () => {
  beforeEach(() => {
    ...
    api.fetchAgoraToken.mockRejectedValue(new Error("agora-token 500"));
    api.endCall.mockResolvedValue(undefined);
    api.sendHeartbeat.mockResolvedValue(undefined);
  });

  it("calls endCall('call-1', 'failed') when startCall resolves but fetchAgoraToken rejects", async () => {
    render(<App />);
    ...
    // Trigger the call: onCall → onStartCall → startCall resolves, fetchAgoraToken rejects.
    tap.click();

    await waitFor(() => {
      expect(api.endCall).toHaveBeenCalledWith("call-1", "failed");
    });
    // The token fetch failed before joinChannel could run.
    expect(agora.joinChannel).not.toHaveBeenCalled();
  });
});
