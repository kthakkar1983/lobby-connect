 ...
 *   - select:  .select(...).eq(...).is(...).maybeSingle() -> { data }
 *   - insert:  .insert(values) -> { error }               (awaited directly)
 *   - update:  .update(values).eq(...).is(...) -> { error } (awaited, chained)
 * Spies record what was written so tests assert on writes, not the builder.
 */
function mockAdmin(script: {
  ...
  const admin = {
    from: (table: string) => ({
      ...
      update: (values: unknown) => {
        ...
        return {
          eq: () => ({
            is: () => Promise.resolve(script.update?.[table] ?? { error: null }),
          }),
        };
      },
    }),
  ...
  return { admin, insertSpy, updateSpy };
}
