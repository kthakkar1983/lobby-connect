...
## 2026-07-04 (third session) — GATE 3.1 FULL PASS both machines → staging smoke systematically debugged → Agora-cert-on-staging DECLINED (LiveKit next) → fix loop → PR → PROD smoke

Resumed from the build-gates handoff; Kumar delivered both gate reports. **GATE 3.1 = PASS on Mac (~15:14 CDT: 0.4/0.5/0.7s client-clock, tab-hidden, 360s box delayed send exactly on schedule) AND Windows PC (~17:53 CDT: 0.4/0.4/0.6s)** — on BOTH machines the concurrently-open tile tab logged max tick gap 60.0s (THROTTLED) during the 360s wait while the push landed sub-second → **push wake proven independent of tab throttling (the pivot's thesis); Phase C GO, Plan B (all-shift tile) retired.** Results stamped in plan Task 3 (`2380a1a`). Product finding (a ring Kumar couldn't take rang out full-length): **ring-SILENCE control required** on agent + admin ringing surfaces (+ the Phase-D tile) — local-mute only, stays answerable, resets per ring.

**Phase-B staging smoke attempt 1 (~18:30 CDT) = "no one available" on every kiosk tap → systematic-debugging found a 3-layer STAGING-ENV failure (not the new Phase-B code):** (1) **`AGORA_APP_CERTIFICATE` missing on the Coolify staging portal** (checked container env via SSH; `AGORA_APP_ID` + VAPID present) → `/api/agora/token` 500 → the kiosk bails to apology seconds AFTER `startCall()` created the row; (2) **latent kiosk bug (prod `main` too):** `onStartCall`'s catch never `endCall`s the row it created → ghost answerable ring under an apology screen; Kumar answered from the admin account, the overlay's token fetch also 500'd, the row stuck IN_PROGRESS → **0016's one-active-per-property index correctly 409'd every later tap** (zero rings; 30-min reap horizon) — cleared reaper-style via MCP (FAILED + duration; admin presence self-corrected next heartbeat, S3 verified live); (3) **migration 0018 was never back-applied to staging** (built 06-21 < v1.2 06-28) → realtime subscribe authz-denied → rings surfaced only on the 60s poll / focus refetch ("rang only after I switched tabs") — **0018 applied to staging via MCP.** Attempt 1 DID prove the new PropertyCard rings live in a real browser. **DECISION (Kumar): NO Agora cert on staging** — LiveKit lands in Phase 4; staging video-connect testing resumes post-swap → staging connect beats WAIVED; **PROD smoke post-merge = the real Phase-B verification**; fix loop moves BEFORE the PR. Plan Task 10 annotated (`ce422a2` + decision commit).

**NEXT SESSION: (1) fix loop on the branch (ring-silence control + kiosk catch-leak `endCall("failed")` fix, subagent-driven) → (2) PR to `main` → (3) PROD two-call smoke (checklist in the handoff; audio card's first live exercise; try Silence; feel out Decline-gone; End every answered call) → (4) Phase C (Task 11 = 0019; apply to staging AND prod — the 0018 lesson). START HERE: `docs/handoffs/2026-07-04-phase3-post-gates-handoff.md`** (also carries the Phase-2 real-night close-out + July-10 soak checklists). Phase-1 soak + Phase-2 real-night gate untouched this session.

## 2026-07-05 (fourth session) — PHASE B DONE + PROD-SMOKE VERIFIED → build Phase C next

Resumed from the post-gates handoff. **Fix loop shipped subagent-driven (fresh implementer per fix + 3 adversarial reviews = ZERO must-fix):** **(a) ring-silence control** — Silence toggle on every ringing surface (agent PropertyCard, admin fleet cards, unmatched-ring fallback); mutes the LOCAL audio ringer only, card keeps ringing visually + stays answerable, auto-resets next ring. **SDK-verified finding: `device.audio.incoming(false)` cannot stop a LIVE Twilio ring** (the flag is read once when the call arrives) → the softphone now owns its own `/sounds/ring.mp3` element with the built-in ring disabled (commit `b2d6513`). **(b) kiosk catch-leak** — `onStartCall`'s catch now `endCall(id, "failed")`s the row it created; new kiosk jsdom test harness (jsdom + testing-library added) (commit `ac0a9b8`). **PR #27 merged to `main`/prod.**

**PROD two-call smoke PASSED (Kumar, on prod):** **audio full pass** (no double-ring — the built-in-ring-disable confirmed; rang on a backgrounded tab; Silence muted the local ring while staying answerable; answered a silenced call). **Video pass** (card rings; Answer connects to the kiosk; Silence works). **Two smoke findings, both root-caused via systematic-debugging + fixed on `main`:** (1) *transient "Something went wrong" on the first video answer* = a post-deploy **ChunkLoadError** (open tab held a stale chunk map; the video overlay is `next/dynamic` lazy-loaded → the throw bubbled past the segment boundary to `global-error` because `VideoCallHost` is mounted in the layout) — **self-healed** with no code change; carry-forward = a durable ChunkLoadError→reload guard. (2) *video ring kept blaring ~30s AFTER answering* — root cause: video gated its ring on the server incoming list, which clears the answered call only via a realtime broadcast a just-focused/re-subscribing tab misses → the 60s fallback poll (`INCOMING_VIDEO_FALLBACK_POLL_MS`, avg ~30s); audio never had this (it stops on the local `phase→in-call`). **Fix (PR #28, `f4af480`):** `useIncomingVideoCalls(operatorId, silencedKeys, activeCallId)` excludes the answered id (+ an answered-id set for post-hangup) from the ring/tab-title/returned list, so the ring stops on the LOCAL answer. **`waiting` is `useMemo`'d** — the first cut returned a fresh `.filter()` array that the host's `publishRings` effect depended on → infinite render-loop → OOM the jsdom suite caught (the session's 2nd DEP-HYGIENE loop). Kumar re-smoked: **"rings stop the instant a call is answered."**

**⚠ DEP-HYGIENE was the dominant failure mode this session (2 render-loops):** (a) publisher effects depend on stable dispatchers, never on `surface`; (b) a hook returning an array that feeds a consumer's effect MUST memoize it. The jsdom loop-guard/OOM tests are load-bearing.

**Gates green on `main`:** portal 545 node + 79 jsdom · kiosk 27 · typecheck · root lint · check:routes · both builds. VAPID keys live in Vercel prod + Coolify staging.

**Preview-deploy note (Kumar asked):** the Vercel PR preview points at the STAGING Supabase (`Preview (staging)`-scoped env) + gets no Twilio webhooks + no Agora cert → the interactive call-path smoke can never run there; it's only a build-gate. Left ON (Kumar's call). Call-path smoke is always PROD-only.

**NEXT SESSION: build PHASE C (push productionized + Go-on-duty/End-shift) — Task 11 first = migration `0019_push_subscriptions` + `lib/push/{targets,send}.ts` + subscription route; apply 0019 to STAGING via MCP when built AND PROD at merge (the 0018 lesson); `pnpm gen:types` after (local `supabase start` + CLI 2.101.0). Phase C is what closes the backgrounded/off-home ring limits the smoke surfaced (both expected/by-design). START HERE: `docs/handoffs/2026-07-05-phase3-phaseC-kickoff-handoff.md`.** Carry-forward: ChunkLoadError→reload guard. Phase-1 soak (~07-10) + Phase-2 Dilnoza real-night gate untouched this session.
