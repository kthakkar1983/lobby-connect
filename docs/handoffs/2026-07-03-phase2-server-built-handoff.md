...
> **Progress (2026-07-03, same day): H1–H5 ALL DONE — only the real-night gate remains.** DNS verified · keypair + export string in PM · four peers key-authenticated on our hbbs: Kumar's Mac (`139513354`) · the pilot hotel PC (`511505435`, repointed on-site) · India admin (`250885235`) · **pilot agent Dilnoza (`428868591`, India mobile/CGNAT)**. Admin sessions rode hbbr (verdict: "snappy" on the relayed worst case); **Dilnoza's and Kumar's hotel-PC sessions went DIRECT P2P (no hbbr trace)** — relay is fallback-only, as designed. Zero auth failures. **Remaining: Dilnoza works a real night through our relay → stamp Phase-2 DONE + tag `plan-phase2-relay-complete`.** Rollback stays: clear Network fields on both ends (Kumar on-site for the hotel PC).
