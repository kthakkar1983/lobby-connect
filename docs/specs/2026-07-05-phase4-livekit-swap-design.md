...
| D14 | ~~Rollout: staging flips to LiveKit immediately; merge to `main` is prod-inert (flag=agora); prod flip is a separately-timed env-only change; Agora path retained until LiveKit survives ≥1 week of real nights; then strip~~ **AMENDED 2026-07-06 (Kumar, post-staging-smoke): the prod flip on Vercel never happens.** Vercel freezes as a warm standby (still Agora); PR #29 merges with deploys disabled; **Agora strips from `main` immediately**; real-nights proof + rollback ride the Phase-5 blue-green cutover (whole-stack pointer swap to/from the standby) | The staging half of D14 executed as designed (staging = pure LiveKit, smoke PASS 2026-07-06). Kumar traded the env-flag partial rollback for total-instant-rollback via the standby + an immediately-clean trunk; recorded in the migration plan Phase 4/5 amendments + sequencing rationale |
