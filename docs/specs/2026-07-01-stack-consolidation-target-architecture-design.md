...
> **Amended 2026-07-04 (Phase-3 brainstorm — `docs/specs/2026-07-04-phase3-workspace-design.md` supersedes the two blocks above/below):** Kumar rejected the all-shift tile as solving two problems at once. The tile is now **call-scoped** — opens on the Answer gesture, guest-video-first (kiosk eye contact while in RustDesk), dies at hang-up — and **always-rings is owned by a push-first audible path** (service-worker wake → loud primed ringtone from the buried tab; OS toast = best-effort context; "as long as they can hear it ring… they can always switch quickly to the lc tab and answer from the dashboard"). Gate 3.0 evidence (both OSes) stands; Gate 3.1 verifies the push chain.
