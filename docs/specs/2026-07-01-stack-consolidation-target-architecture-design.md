...
**Backups/DR:**
- DO auto-backups, weekly, 20% of droplet price (+$9.60/mo) ([DO pricing](https://www.digitalocean.com/pricing/droplets)); snapshot manually before each migration phase.
...
- **Custom domain (new requirement):** e.g. `app.<domain>` (portal), `kiosk.<domain>`, `staging.<domain>`, `relay.<domain>` (RustDesk), `livekit.<domain>`/`turn.<domain>`. Registrar ~$10–15/yr (*estimate*). Cutover updates: Twilio webhook URLs, Supabase Auth site/redirect URLs, kiosk `?t=` links (regenerate), cross-app env URLs, Sentry allowed origins.
