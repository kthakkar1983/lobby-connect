...
- **Off-duty resync (2026-07-06 smoke fix):** an off-duty tab beats nothing, so it re-reads the hydration GET on the beat cadence (interval + focus) and applies the answer — READ-ONLY, so it can never resurrect a shift; the gate stays the enforcement. `accepting` is applied before the follow-up beat so the beat can't post a stale default over a real AWAY. Both directions now converge within one beat: on→off via gated beats, off→on via resync.
