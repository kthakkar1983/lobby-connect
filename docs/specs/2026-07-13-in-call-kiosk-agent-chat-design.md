...
- **Algorithm (hardened 2026-07-14; Luhn gate DROPPED 2026-07-14 after prod smoke):** find maximal runs of `[0-9 .-]` (space, dot, hyphen separators), strip separators → candidate digits, then mask the run iff **either** (a) the whole run is length ∈ [13,19] — **Luhn is NOT required** (the base rule), **or** (b) the run is 20–25 digits and contains a Luhn-valid 13–19 window **anchored at the start or end** with ≤6 leftover digits on the other side — i.e. a PAN glued to a short expiry/CVV. **Why no Luhn on the base rule:** every real card passes Luhn, so the gate never caught a real card the length rule doesn't — but it *let through* a fat-fingered real card (≈15 of 16 real digits) and card-shaped test input (the smoke found `4111 1111 1111 1234`, a Luhn-failure, printed in full). Masking every 13–19 digit run is the "when in doubt, mask" PCI-firewall posture. **Accepted cost:** a genuine 13–19 digit number a guest might type (an international phone with country code, some 13-digit itinerary numbers) is also masked — recoverable, since the guest is on live video and can read it aloud. The 20–25 case keeps its Luhn anchor so a long non-card string isn't masked by mere length. Issuer prefix (Visa / MC / Amex / Discover / …) is **not** required either. *History: original rule was whole-run Luhn-only; hardened to catch dot separators + a PAN glued to expiry/CVV; then the base Luhn gate was dropped per the smoke above. Residual accepted edge: two full cards mashed into one >25-digit run.*
...
- **Proven by a TDD table:** positives (real test PANs, spaced/dashed, **and card-shaped Luhn-failures**) masked; negatives — everything **under 13 digits** (house numbers, ZIP 5/9, US phone 10–11, room numbers, dates, short order/confirmation numbers) — pass untouched. A 13–19 digit non-card run is now intentionally masked (accepted trade-off, above).
...
- **D7 — PCI = active kiosk-side redaction** (any 13–19 digit run, pre-publish; **hardened 2026-07-14** for dot separators + PAN-glued-to-expiry/CVV, then the **base Luhn gate DROPPED 2026-07-14 after prod smoke** so a card-shaped Luhn-failure / fat-fingered card is also masked — see §6) **+ passive notice** — both layers.
