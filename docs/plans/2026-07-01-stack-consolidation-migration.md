...
- **Hold** (audio via the Twilio Conference seam; held state on card + tile + overlay; **video-hold wiring deferred to Phase 4/LiveKit — no Agora plumbing**, spec D9). Pre-warm = credential fetch on answer (spec D10).
- Own brainstorm→spec→plan cycle; the 911 path gets the usual byte-level review treatment.

**Rollback:** normal git revert; no infra involved. **Done when (amended 2026-07-04 per the spec):** prod smoke — answer on the expanded card (agent + covering admin) · one-click Connect to the real hotel PC **from the card and from inside a live call (D12)** · admin-connect to a non-covered property · hold/resume on an audio call · **loud ring lands with the browser minimized behind fullscreen RustDesk (push path), toast observed** · call tile opens on Answer and carries the call over RustDesk.
