...
## Task 11: staging cutover + smoke — **[HUMAN + CONTROLLER — not a subagent task]** — DONE, SMOKE PASSED 2026-07-06

**Record:** Envs + carve-out label in via Coolify UI (label recovery incident: the Readonly checkbox REGENERATES labels when re-checked, wiping custom lines — restored byte-identical from the captured original; **leave Readonly UNCHECKED forever**; noted in runbook §10 amendment below). `staging` fast-forwarded `c07a965→82e1aea`; deploy verified: new carve-out live on the container · boot log clean of Agora/LiveKit warnings (D15 proof; only the known no-Twilio-on-staging line) · root still basic-auth'd · `/api/video/token` answers 404 JSON with no Basic challenge. **SMOKE (Kumar): PASS** — kiosk tap → card ring + OS push (browser minimized behind RustDesk) → notification click → Answer → **video connected through box LiveKit** (two-way A/V) · captions on guest speech · hang-up finalizes + clears push · guest-side ending clean · **busy-webcam → connected audio-only with warning** · End-shift/AWAY silence re-checks pass. One finding: agent-tab reload mid-call ends the call for both sides — **expected v1 semantic, byte-identical on Agora (kiosk finalizes on agent-left); NOT a Phase-4 regression**; D9 is zombie-prevention, not resume; logged in `docs/v2-backlog.md` ("Mid-call resume across an agent reload").
