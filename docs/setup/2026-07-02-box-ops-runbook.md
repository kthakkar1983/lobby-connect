...
- One shared user `staging`; bcrypt hash lives in a Traefik label on both apps (Coolify → app → **General → Container Labels** — NOT Advanced, which was mis-recorded here originally), NOT in the repo.
- **⚠ Readonly-checkbox trap (learned 2026-07-06, Phase 4):** the "Readonly" toggle above the labels text area means "Coolify regenerates these labels itself" — RE-CHECKING IT WIPES every hand-added line (auth middleware, carve-outs router) with the auto-generated default set, saving immediately without a Save click. It must stay UNCHECKED on both apps, forever. Labels apply at container (re)creation, so a wipe only bites at the next deploy — if it happens, restore from the current live container (`docker inspect <container> --format '{{json .Config.Labels}}'`) before redeploying.
...
- **Phase 4 (LiveKit): BUILT — see §13.** The TURN/443 question resolved itself: NO TURN at launch (public-IP SFU + ICE/TCP 7881 fallback; enable-later seam in the Phase-4 spec §2.5).
...
- **Post-gate tuning experiment (recorded 2026-07-03, deliberately NOT done during cutover):** the client toggle "Enable IPv6 P2P connection" (Settings → General), enabled on BOTH ends of the India↔hotel pair, may convert relayed sessions to direct (India ISPs have high IPv6 adoption; v6 needs no NAT punch — *unverified read, test deliberately*). Try only after the real-night gate; one pair at a time; rollback = toggle off. Server side is already v6-ready (box has IPv6; DO fw + ufw relay rules cover v6; `relay.` is A-only today — add an AAAA only if a test shows it's needed).
...
Design: `docs/specs/2026-07-05-phase4-livekit-swap-design.md` · plan: `docs/plans/2026-07-05-phase4-livekit-swap.md`.

- **What/where:** `livekit/livekit-server:v1.13.3` as plain compose at `/opt/livekit/` (deliberately NOT Coolify — same disjoint-failure-domain rule as the RustDesk relay; host networking). Repo source of truth: `ops/livekit/` (deploy = scp + `docker compose up -d`). Config `/opt/livekit/livekit.yaml` holds the two API keypairs (`lc_prod`, `lc_staging`) — PM + Mac backup; never committed (the repo carries `livekit.yaml.example`).
- **Ports:** 7880/tcp signal — firewall-BLOCKED externally; Coolify's Traefik proxies `livekit.lobby-connect.com` → `http://159.203.124.112:7880` via the dynamic config `/data/coolify/proxy/dynamic/livekit.yaml` (a file drop there = what the Coolify UI's Dynamic Configurations screen writes; Traefik watches the dir) · 7881/tcp ICE fallback (OPEN; must not sit behind a proxy) · 7882-7885/udp media mux (OPEN). **No TURN** — enable-later keys recorded in the Phase-4 spec §2.5; revisit only on evidence of a client that cannot connect.
- **ufw as-built gotcha (2026-07-05):** the proxied-signal allow is `from 10.0.1.0/24 to any port 7880 proto tcp` — **Coolify's proxy network is `10.0.1.0/24`, NOT Docker's default 172.16/12** (a 172.16/12 rule silently drops Traefik → 502). If Coolify ever re-nets the proxy, re-check `docker inspect coolify-proxy` and fix this rule.
- **Kernel tuning (as-built 2026-07-05):** `net.core.rmem_max=5000000` set + persisted at `/etc/sysctl.d/99-livekit.conf` — LiveKit boot-WARNs "UDP receive buffer is too small for a production set-up" below that.
- **Health:** on-box `curl -s http://127.0.0.1:7880/` → `OK` (406 `Not Ready` = unhealthy). From outside: `curl -s https://livekit.lobby-connect.com/` → `OK` (also proves DNS + Traefik route + LE cert).
- **Logs:** `docker logs livekit --tail 100` (json-file, capped 3x10MB).
- **Restart / update:** `cd /opt/livekit && docker compose up -d`. Version bump: snapshot the droplet → edit the image tag in `ops/livekit/compose.yaml` (repo first) → scp → `up -d`.
- **Key rotation:** edit `keys:` in `/opt/livekit/livekit.yaml` → `docker compose up -d` → update the matching portal env (`lc_staging` → Coolify staging; `lc_prod` → Vercel prod) → PM + credentials register.
- **Staging/prod:** ONE instance serves both, distinguished by keypair. Keypairs do NOT namespace rooms (any valid token can name any room) — acceptable because room names are unguessable `call_<uuid32>` and tokens are minted only for live-call rows by the authed `/api/video/token` route.
- **Incident ("video won't connect"):** follow the staging debug order (handoff 2026-07-05): targeting/assignment → card ring → push → ONLY THEN the video leg. For the video leg: 1) the health curls above; 2) `docker logs livekit --tail 100` for join/ICE errors; 3) ports from outside `nc -vz 159.203.124.112 7881` + `nc -vzu 159.203.124.112 7882`; 4) token mint sane? (`/api/video/token` returns `provider:"livekit"` + `url`); 5) rollback for PROD = flip `VIDEO_PROVIDER` back to `agora` on Vercel + redeploy (env-only, no code).
