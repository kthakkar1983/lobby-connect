...
| 7b | Owner self-service writes — kiosk-content inline editing + playbook upload/view + incident resolve. **Approach A** (locked 2026-06-03 brainstorm): owner `UPDATE` RLS on `properties`/`incidents` + `BEFORE UPDATE` column-guard triggers (RLS is row-level only); playbook is the lone service-role route. Migration 0010 adds `incidents.resolution_note`. Domain seam: owner owns guest-facing content, admin owns ops. Spec: `docs/specs/2026-06-03-07b-owner-writes-design.md` · Plan: `docs/plans/2026-06-03-07b-owner-writes.md` (9 TDD tasks) | **spec + plan written, not yet built** |
| 8 | Observability | — |
