...
| 8 | Observability — Sentry (both apps, PII-scrubbed) + `@vercel/analytics` + admin `/audit` viewer + admin `/status` health page driven by a generic `health_signals` heartbeat registry (Twilio webhook + cron self-report) and live Supabase/Sentry probes. Single plan, build order Sentry → /audit → /status. Migration 0011. **SPEC + PLAN WRITTEN, not yet built** — 14 TDD tasks; T7 has a manual Sentry-setup checkpoint (org `lobby-connect` exists, no project yet). Spec: `docs/specs/2026-06-03-08-observability-design.md` · Plan: `docs/plans/2026-06-03-08-observability.md` | — |
